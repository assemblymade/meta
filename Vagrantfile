...
raise <<-EOF unless File.exists?("vagrant.yml")
  ...
  Hi! Before getting started with Vagrant and Meta
...
raise <<-EOF unless File.exists?('.env')

  Hi! Before getting started with Vagrant and Meta
  you'll need to setup the `.env`. There should
  be a file `.env.example` that you can use as
  a base reference. Copy the `.env.example` to
  `.env` to get started.
...
$box = 'meta'
$box_url = 'https://s3.amazonaws.com/asm-assets/meta_v1.box'
...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.network :private_network, ip: '192.168.63.82' # 192.168.me.ta
