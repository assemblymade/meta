...
module.exports = {
  ...
  deselectCurrentBounty() {
    Dispatcher.dispatch({
      type: ActionTypes.BOUNTY_DESELECT_CURRENT
    })
    Dispatcher.dispatch({
      type: ActionTypes.DISCUSSION_CLOSE
    })
    Dispatcher.dispatch({
      type: ActionTypes.NEWS_FEED_ITEM_CLOSE
    })
  },

  fetch(product_id, number) {
    Web.get(Routes.product_task_path({ product_id: product_id, id: number}), data => {
      let {
        assets,
        bounty,
        heartables,
