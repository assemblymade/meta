namespace :stages do
  ...
  task :create_stages => :environment do
    ...
    idea = Stage.create!({name: "Idea", description: "Every great business begins as an idea"})
    ...
    recruitment = Stage.create!({name: "Recruitment", description:
      "Find the partners who can help build something amazing"})
    ...
    setup = Stage.create!({name: "Get Started", description: "Lay the groundwork for a thriving project."})
    ...
    mvp = Stage.create!({name: "Build an MVP", description: "Build a minimum viable product.  Create something beautiful."})
    launch = Stage.create!({name: "Launch", description: "Take your product to the people.  Let it soar."})
    grow = Stage.create!({name: "Grow", description: "Take your product to the next level. Iterate, improve, and grow. The world is yours for the taking."})
