...
# and then further defaced by just3ws to create the meta devenv
...
apt-get -y install postgresql-doc-9.3

# Add 'vagrant' role
su -c 'createuser -s vagrant' postgres
su -c 'createuser -s meta' postgres
...
su postgres -c 'createuser -s meta'
...
su - vagrant <<-'EOF'
  ...
  rvm gemset create meta
  rvm use $_RUBY_VERSION --default
  rvm use $_RUBY_VERSION@meta
